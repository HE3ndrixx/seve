...
setup_ssh_config() {
    ...
    if ! grep -q "Host github-$github_label" $ssh_config_path 2>/dev/null; then
        ...
        echo -e "\nHost github-$github_label" >>$ssh_config_path
...
# Clean up old GitHub connection
reset_git_remote() {
    echo "Removing existing Git remotes..."
    git remote remove origin 2>/dev/null
    echo "Old GitHub remote removed."
}
...
setup_and_push() {
    ...
    # Remove any existing remote and reset
    reset_git_remote
    ...
    git commit -m "$commit_message" 2>/dev/null || echo "Nothing new to commit."

    echo "Pushing changes to GitHub..."
    git push -u origin $new_branch --force
    echo "✅ Code successfully pushed to GitHub via SSH!"
}
